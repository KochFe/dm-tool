...
async def test_roll_2d6_plus_3(client: AsyncClient):
    """2d6+3 — verify full response structure and arithmetic."""
    resp = await client.post(ROLL_URL, json={"notation": "2d6+3"})
...
async def test_roll_1d20_no_modifier(client: AsyncClient):
    """1d20 — no modifier present, modifier must default to 0."""
    resp = await client.post(ROLL_URL, json={"notation": "1d20"})
    assert resp.status_code == 200
...
async def test_roll_4d6_minus_1(client: AsyncClient):
    """4d6-1 — negative modifier is correctly parsed and applied."""
    resp = await client.post(ROLL_URL, json={"notation": "4d6-1"})
    assert resp.status_code == 200
...
async def test_roll_1d100_percentile(client: AsyncClient):
    """1d100 — percentile die is a valid die type."""
    resp = await client.post(ROLL_URL, json={"notation": "1d100"})
    assert resp.status_code == 200
...
async def test_roll_1d4_smallest_die(client: AsyncClient):
    """1d4 — smallest standard D&D die is accepted."""
    resp = await client.post(ROLL_URL, json={"notation": "1d4"})
    assert resp.status_code == 200
...
async def test_roll_invalid_notation_abc(client: AsyncClient):
    """'abc' does not match the notation regex — must return 422."""
    resp = await client.post(ROLL_URL, json={"notation": "abc"})
    assert resp.status_code == 422


async def test_roll_invalid_count_zero(client: AsyncClient):
    """'0d6' — count of 0 is below the minimum of 1 — must return 422."""
    resp = await client.post(ROLL_URL, json={"notation": "0d6"})
    assert resp.status_code == 422


async def test_roll_invalid_count_too_high(client: AsyncClient):
    """'101d6' — count of 101 exceeds the maximum of 100 — must return 422."""
    resp = await client.post(ROLL_URL, json={"notation": "101d6"})
    assert resp.status_code == 422


async def test_roll_invalid_sides_unsupported(client: AsyncClient):
    """'2d7' — d7 is not a standard D&D die — must return 422."""
    resp = await client.post(ROLL_URL, json={"notation": "2d7"})
    assert resp.status_code == 422
