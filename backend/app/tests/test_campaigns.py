...
async def test_create_campaign(client: AsyncClient):
    resp = await client.post("/api/v1/campaigns", json={"name": "Lost Mines"})
...
async def test_list_campaigns(client: AsyncClient):
    await client.post("/api/v1/campaigns", json={"name": "Campaign 1"})
    await client.post("/api/v1/campaigns", json={"name": "Campaign 2"})
...
async def test_get_campaign(client: AsyncClient):
    create = await client.post("/api/v1/campaigns", json={"name": "Test"})
    cid = create.json()["data"]["id"]
    resp = await client.get(f"/api/v1/campaigns/{cid}")
    assert resp.status_code == 200
...
async def test_update_campaign(client: AsyncClient):
    create = await client.post("/api/v1/campaigns", json={"name": "Old"})
    cid = create.json()["data"]["id"]
    resp = await client.patch(f"/api/v1/campaigns/{cid}", json={"name": "New", "party_level": 5})
    assert resp.status_code == 200
    data = resp.json()["data"]
...
async def test_delete_campaign(client: AsyncClient):
    create = await client.post("/api/v1/campaigns", json={"name": "ToDelete"})
    cid = create.json()["data"]["id"]
...
async def test_create_campaign_validation(client: AsyncClient):
    resp = await client.post("/api/v1/campaigns", json={"name": ""})
    assert resp.status_code == 422
    resp = await client.post("/api/v1/campaigns", json={"name": "X", "party_level": 25})
    assert resp.status_code == 422
