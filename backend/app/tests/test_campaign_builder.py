...
async def test_create_draft_campaign(client: AsyncClient, auth_headers):
    """Creating a campaign with status='draft' sets status=draft."""
    resp = await client.post(
        "/api/v1/campaigns",
...
async def test_patch_campaign_description(client: AsyncClient, auth_headers):
    """PATCH description persists the field."""
    campaign = await _create_campaign(client, auth_headers, status="draft")
    cid = campaign["id"]

    resp = await client.patch(
        f"/api/v1/campaigns/{cid}",
        json={"description": "A world of endless forests."},
        headers=auth_headers,
    )
